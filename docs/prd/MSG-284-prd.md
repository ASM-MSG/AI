...
- **목표**
  - 암흑·렌즈 가림 영상이 추론 없이 판정되고, 해당 잡의 총 처리 시간이 **같은 조건 정상 잡의
    1/5 이하**로 끝난다. 절대 초가 아니라 비율인 이유는 두 가지다 — ①이 기능의 가치가 원래
    "몇 분의 1이냐"이지 초 수가 아니고 ②절대 초는 인스턴스 사양에 종속돼 하드웨어를 바꾸면
    요건을 다시 써야 한다. 실측: dev EC2에서 31.94초 / 242.50초 = **1/7.6**(§7).
  - 정상 영상 **오탐 0** — 커밋된 샘플 4종(`normal`·`crowd`·`plates`·`hires`)과 한국 클립
    (`kr-faces`·`kr-plates`) 전부 통과.
  - 판정 자체의 비용이 정상 영상 처리 시간의 **5% 미만**. 실측 4.23초 / 242.50초 = 1.7%(§7).
    이 비용은 통과하는 영상 **전부**가 무는 것이라, 무의미 영상 비율이 손익분기(약 2%)를
    넘지 않으면 기능 전체가 순손해다 — 운영 후 탈락률 관측이 필요하다.
- **비목표(스코프 제외)**
...
| 분류 | 요구사항 |
|------|----------|
| 성능 | **상대 기준**(§2): 탈락 잡 총 처리 ≤ 같은 조건 정상 잡의 1/5, 판정 비용 ≤ 정상 잡 처리 시간의 5%. 절대 초는 인스턴스 사양에 종속돼 요건으로 쓰지 않는다 — 초기 PRD가 2초·5초를 **macOS 수치로** 잡았다가 EC2에서 각각 2배·6배 미달로 드러나 이 기준으로 바꿨다(2026-08-02). 측정은 dev EC2에서만 유효 |
...
sequenceDiagram
    ...
    alt 탈락 (암흑·렌즈 가림)
        ...
        Note over W: 추론·하이라이트 건너뜀 (dev EC2 1080p 실측 ~32초, 정상 잡의 1/7.6)
        BE->>API: GET /jobs/{id}
