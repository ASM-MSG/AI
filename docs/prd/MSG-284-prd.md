...
- **목표**
  - 암흑·렌즈 가림 영상이 추론 없이 판정되고, 해당 잡의 총 처리 시간이 **1080p 입력 기준
    5초 이내**로 끝난다(현재 ~187초). 4K 입력은 다운스케일 시간이 그대로 더해진다 —
    판정은 다운스케일 **후**에 하므로(§7 실측 근거) 그 시간을 아낄 수 없다. 이 경우에도
    ~187초 대비 20배 이상 단축이므로 목적("3분을 안 쓴다")은 달성된다.
...
| 분류 | 요구사항 |
|------|----------|
| 성능 | 판정 비용 2초 이내(1080p 다운스케일본 기준), 탈락 잡 총 처리 **1080p 입력** 5초 이내 — 4K 입력은 다운스케일 시간이 더해진다(§2). dev EC2 t3.small 실측 기준 — macOS 측정 무효 |
