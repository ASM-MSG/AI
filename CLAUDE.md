...
- 완료: MSG-142(측정) · MSG-143(ADR) · MSG-144(모델 선정) ·
  ...
  **BE 대응(MSG-286) 전까지 탈락 잡은 BE에서 PT30M 뒤에야 FAILED로 수렴한다** — 409를 "완료 전"으로
  ...
  `results/MSG-367-identity.json`. **EC2 전후 실측(`results/MSG-367-report.md`)은 머지 후 남음**)
- 남은 것: **MSG-367 EC2 전후 실측** — 머지 후 dev AI 인스턴스에서 유휴 API 왕복 3회 중앙값 +
  피크 메모리·스왑 (성공 기준 4행, 방법은 `docs/MSG-367.md` 검증 계획 4) ·
